...
if nz != 0:
  ...
  for ix in range(nx):
    for iy in range(ny):
      for iz in range(nz):
        ...
        z = zmin + (iz + 0.5) * zsize
        d = denst2d[ix, iy, iz]
        print ("%-15.3f %-15.3f %-15.3f %-15.8f" %(x, y, z, d))
  ...
  # Load the (x, y) values
  lines = np.loadtxt(inp, dtype=np.float64, comments='#', usecols=(0,1), unpack=False)
  
  denst2d = np.zeros([nx, ny])
